...
- Keep the v2 flat palette (`black=43`, `gray=154`, `white=220`) matched to the four-source production-resolution means of its material ranges (`20-58`, `103-186`, `170-252`), and retain `crispEdges` material rendering to suppress patch-seam moiré; changing either requires remeasurement and full v2 regeneration.
- Keep v2 face-figure shadows inward and downward: both normalized displacement components must exceed `0.020`, the right face mirrors the left horizontal offset, and v1 directionality remains unchanged.
- Keep fixation markers out of stimulus PNG/SVG files; experiment scripts may overlay fixation dynamically.
- Keep generated phase-scrambled masks as optional assets only; do not schedule them in the default v2 experiment unless a backward-masking block is explicitly approved.
